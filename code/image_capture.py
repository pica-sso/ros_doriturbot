...
class ImageCapture:
    def __init__(self):
        ...
        self.pos_y = 7.7
        self.theta = 3.14
        self.image = np.empty((480, 640), dtype=np.unit8)
    ...
    def publish_image(self, event=None):
        ...
        msg.header.frame_id = f'x:{self.pos_x:.2f}_y:{self.pos_y:.2f}_a{self.theta:.2f}'
        self.pub.publish(msg)
        rospy.loginfo("image was published")

    def run(self, capture_count=10.0, publish_count=50, duration=1.0):
        cap_timer = rospt.Timer(rospy.Duration(duration / capture_count), self.capture_image)
